...
for epoch in range(600):
    ...
    for batch_idx, data_pair in enumerate(train_loader):
        ...
        for i, x_i in enumerate(x):
            ...
            y_hat = forward_model(x_i.squeeze(0).unsqueeze(-1), adj)
            forward_loss,correct_t,correct_1_t= loss_all(y_i, y_hat.squeeze(-1))
                        
            loss += forward_loss
            correct += correct_t
            correct_1 += correct_1_t
        ...
        loss = loss/x.size(0)
        correct = correct/x.size(0)
        correct_1 = correct_1/x.size(0)
        loss.backward()
        optimizer.step()
    ...
    print("Epoch: {}".format(epoch+1), 
          ...
          "\tLoss: {:.4f}".format(loss),
          "\tCorrect: {:.4f}".format(correct),
          "\tCorrect_1: {:.4f}".format(correct_1),
